...
static void uart0_io_pin_request(struct Serial *serial)
{
    ...
    // hal_gpio_pin_afio_select(GPIOA_BASE,GPIO_PIN_13,UART1_TX);
    // hal_gpio_pin_afio_select(GPIOA_BASE,GPIO_PIN_14,UART1_RX);
    // hal_gpio_pin_afio_en(GPIOA_BASE,GPIO_PIN_13,HAL_ENABLE);
    // hal_gpio_pin_afio_en(GPIOA_BASE,GPIO_PIN_14,HAL_ENABLE);
}
...
static void hw_uart0_init(struct SerialHardware *_hw, struct Serial *serial, uint32_t baudrate)
{
    ...
    g_uart0.init_cfg.over_sampl = UART_OVER_SAMPL_8; // UART_OVER_SAMPL_16
...
static void uart0_send_data_isr(void)
{
...
static void uart0_recv_data_isr(void)
{
    uint8_t ch = 0;
...
static void serial_uart0_isr_callback(void)
{
...
///=====================UART0 IQR Handle===============================///
//extern void hci_uart0_isr_callback(void);
uint8_t uart0_owner_flag = 0; // 0 = xboot, ~0 = hci

void UART0_IRQHandler(void)
{
    if (uart0_owner_flag) {
        //hci_uart0_isr_callback();
    } else {
